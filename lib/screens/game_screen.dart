...
class _GameScreenState extends State<GameScreen>
  ...
  double _speedMultiplier() {
    // Speed increases only when entering a higher level.
    // There is no time-based acceleration inside the same level.
    final level = widget.level.clamp(1, 100);
    final levelSpeed = (1.0 + (level - 1) * 0.012).clamp(1.0, 2.19);

    if (_powerStageActive) {
      return levelSpeed * 1.15;
    }
  ...
  void _checkStageMechanic() {
    ...
    // From level 2 onward: 36, 72, 108, ...
    //
    // This keeps Power Stage rewarding without triggering too often.
  ...
  void _activatePowerStage() {
    ...
    // Power Stage does NOT grant Fire Ball.
    // Fire Ball remains available only through its normal power-up.
  ...
  void _loseLife() {
    ...
    // First consume the normal red lives.
    if (_lives > 0) {
      _lives--;
    ...
    // All 3 red lives are gone.
    // Now consume a saved yellow extra life.
    if (widget.save.extraLives > 0) {
      widget.save.setExtraLives(widget.save.extraLives - 1);

      // Give the player one temporary red life to continue.
      _lives = 1;
    ...
    // No red lives and no yellow extra lives remain.
    _lives = 0;
    _saveBrickState();
    _gameOver = true;
    _controller.stop();
  ...
  void _watchAdToContinue() {
    ...
    AdService.instance.showRewarded(
      onReward: () {
        ...
        // Rewarded ad grants one persistent yellow extra life.
        ...
        if (mounted) {
          ScaffoldMessenger.of(context)
            ...
            ..showSnackBar(
              const SnackBar(
                content: Text('Reward granted! +1 🟡 Yellow Life'),
                duration: Duration(seconds: 2),
              ),
            );
        }
...
class _NeonGamePainter extends CustomPainter {
  ...
  void _drawPowerStage(Canvas canvas, Size size) {
    ...
    if (powerStageAnnouncementTimer > 0 && !paused) {
      ...
      _centerText(
        canvas,
        'ACTIVE',
        Offset(size.width / 2, size.height * .50 + 38),
        13,
        NeonColors.yellow,
      );
    }
  ...
  void _drawCenterNeonPause(Canvas canvas, Size size) {
    ...
    // Dark glass center.
    canvas.drawCircle(center, radius, Paint()..color = const Color(0xDD05020D));
    ...
    // Neon play symbol.
    final path = Path()
      ..moveTo(center.dx - 8, center.dy - 13)
      ..lineTo(center.dx + 14, center.dy)
      ..lineTo(center.dx - 8, center.dy + 13)
      ..close();
  ...
  void _drawPaddle(Canvas canvas, Size size) {
    ...
    // MAIN PADDLE — Rainbow Neon Design

    final radius = const Radius.circular(14);
    final paddleRRect = RRect.fromRectAndRadius(rect, radius);
    ...
    canvas.drawRRect(paddleRRect, Paint()..shader = rainbowShader);
    ...
    final inner = Rect.fromCenter(
      center: Offset(center.dx, center.dy + height * .02),
      width: width * .78,
      height: height * .42,
    );
    ...
    canvas.drawRRect(
      ...
      Paint()..color = const Color(0xCC09051A),
    );
    ...
    canvas.drawLine(
      Offset(center.dx - width * .30, center.dy),
      Offset(center.dx + width * .30, center.dy),
      energyPaint,
    );
    ...
    // Crisp energy line.
    canvas.drawLine(
      Offset(center.dx - width * .24, center.dy),
      Offset(center.dx + width * .24, center.dy),
      Paint()
        ..color = Colors.white
        ..strokeWidth = 1.2
    ...
    canvas.drawCircle(
      Offset(center.dx - width * .43, center.dy),
      2.5,
      capPaint,
    );
